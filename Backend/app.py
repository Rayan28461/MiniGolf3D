...
# Updated background task to periodically check shot counts and wait for move before resetting.
async def monitor_shot_counts():
    ...
    while True:
        ...
        if agent_shot_counts and all(shots == 0 for shots in agent_shot_counts.values()):
            print("All agents exhausted shots for generation {}. Triggering immediate reset...".format(current_generation))
            reset_pending = True
            
            # CHANGED: Immediately send reset request without waiting for move signal
            try:
                print("Sending immediate /reset request to Unity...")
            ...
            current_generation += 1
            print("Generation advanced to {}.".format(current_generation))
            agent_shot_counts = {}
            movePlayed = False
            reset_pending = False
        else:
...
async def agent_count(data: AgentCountData):
    ...
    print(f"Received agent count from Unity: {data.agent_count}. Shot counts reset: {agent_shot_counts}")
...
@app.post("/move_played")
async def move_played_endpoint():
    global movePlayed
    movePlayed = True
    print("Move played signal received from Unity.")
